...
import org.apache.calcite.rex.{RexCall, RexNode, RexVariable}
...
object JoinPlanUtil {
  ...
  /** Checks whether a join has equi-predicates. */
  def hasEqualityPredicates(joinInfo: JoinInfo): Boolean = {
    !joinInfo.pairs().isEmpty
  }

  /**
    * Checks whether the predicates can be replaced with a Calc followed by an equi-predicate.
    * (e.g., replace =($1, -($0, 1)) with $2=[-($0, 1)] and =($2, $1)).
    *
    * @return true if the conditions can be replaced with equi-ones.
    */
  def withEquiRexCall(join: LogicalJoin, joinInfo: JoinInfo): Boolean = {
    val remainCond = joinInfo.getRemaining(join.getCluster.getRexBuilder)
    ...
    def checkCond(call: RexNode, andBranch: Boolean): Unit = call match {
      ...
      case x: RexCall if x.op == SqlStdOperatorTable.EQUALS =>
        // Found a predicate with the equality comparator.
        x.operands.asScala.foreach {
          // The predicate references a field.
